...
void imuMagCallback(const sensor_msgs::MagneticField::ConstPtr &mag_msg) {
  ...
  Vector3 field = odom_tracker_.processMagMessage(
      Vector3(mag_msg->magnetic_field.x, mag_msg->magnetic_field.y,
...
void imuCallback(const sensor_msgs::Imu::ConstPtr &imu_msg) {
  odom_tracker_.updateIMU(
      Quaternion(imu_msg->orientation.x, imu_msg->orientation.y,
                 imu_msg->orientation.z, imu_msg->orientation.w));
}

void arduinisCallback(
    const grudsby_lowlevel::ArduinoResponse::ConstPtr &ard_msg) {
...
void gpsOdomCallback(const nav_msgs::Odometry::ConstPtr &gps_msg) {
  Vector3 position =
      Vector3(gps_msg->pose.pose.position.x, gps_msg->pose.pose.position.y,
              gps_msg->pose.pose.position.z);
  Quaternion orientation = Quaternion(
      gps_msg->pose.pose.orientation.x, gps_msg->pose.pose.orientation.y,
      gps_msg->pose.pose.orientation.z, gps_msg->pose.pose.orientation.w);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber gpuOdomSub =
      n.subscribe("odometry/gps", 1000, gpsOdomCallback);

  ros::Subscriber ardSub =
      n.subscribe("grudsby/arduino_response", 1000, arduinisCallback);
