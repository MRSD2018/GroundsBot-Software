...
void OdomHeading::updateOdom(unsigned int left_wheel, unsigned int right_wheel) {
  if (first_odom_received_) {
    double leftDist = WHEEL_RAD * (left_wheel - left_wheel_odom_) * 2.0 * M_PI / 4096.0;
    double rightDist = WHEEL_RAD * (right_wheel - right_wheel_odom_) * 2.0 * M_PI / 4096.0;
    Quaternion zHalfRot = Quaternion::FromEuler(0,0,atan2(rightDist-leftDist, WHEELBASE_LEN)/2.0);
  ...
  if (first_gps_received_) {
    ...
    if (dist > GPS_DIST_THRESHOLD) {
      ...
      if (abs(odomDist - dist) < GPS_ODOM_DIFF_THRESHOLD) {
        th_heading_estimate_ = atan2(odom_pos_.Y,odom_pos_.X)-atan2(deltaGPS.Y,deltaGPS.X);
        odom_pos_= Vector3(0.0,0.0,0.0);
        odom_ori_yaw_ = 0;
        last_gps_pos_ = pos;
        last_gps_ori_ = ori;
...
Vector3 OdomHeading::processMagMessage(Vector3 magVector) {
  if (heading_estimate_established_) {
    Vector3 ret = Vector3(0.19*cos(th_heading_estimate_ + odom_ori_yaw_ + M_PI / 2.0), 0.19*sin(th_heading_estimate_ + odom_ori_yaw_ + M_PI / 2.0), 0.0);
    return ret;
  }
  else {
    return magVector;
